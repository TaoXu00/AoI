...
def transmission_algorithm(b_real, b):
...
data_size=random_data_generator(M, 1500, 1500)

DS=data_size

#compute the path and the AoI of the optimal trajectory
...
b_hat_list = [[0 for _ in range(N)] for _ in range(N_trials)]

for i in range(N_trials):

    #generate the distribution of the datarate
    b_real=[]
    for j in range(N):
    ...
    AoI, list_tx_reorder, time_spent2, transmission_position=transmission_algorithm(b_real ,b_mod)
    if i<20:
        GR.draw_line_between_points(i+10,list_visited_sensor_ordered,sensor_positions[:M],list_tx_reorder, time_spent2, upper_bound, lower_bound, "Sol UCB: "+str(i), 1000,sum(AoI),  bit_rate_list[:M] )
    ...
    for j in range(N):
        b_hat_list[i][j] = copy.deepcopy(b_hat[j])
